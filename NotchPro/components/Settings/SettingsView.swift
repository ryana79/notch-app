...
struct ProductivitySettings: View {
    ...
    @Default(.showWeatherGlance) var showWeatherGlance
    ...
    var body: some View {
        Form {
            Section {
                ...
                Defaults.Toggle(key: .showWorkoutGlance) {
                    Text("Gym workout tracker")
                }
                .onChange(of: showWorkoutGlance) {
                    if showWorkoutGlance {
                ...
                Defaults.Toggle(key: .showSystemStats) {
                    Text("System stats (CPU / RAM)")
                }
                .onChange(of: showSystemStats) {
                    if showSystemStats {
                ...
                Defaults.Toggle(key: .showWeatherGlance) {
                    Text("Weather glance")
                }
                .onChange(of: showWeatherGlance) {
            ...
            Section {
                Defaults.Toggle(key: .glassmorphismEnabled) {
                    Text("Glass effect on expanded notch")
                }
                Defaults.Toggle(key: .accentGlowEnabled) {
...
struct MoreFeaturesSettings: View {
...
struct GeneralSettings: View {
    ...
    @Default(.layoutPreset) var layoutPreset
    ...
    var body: some View {
        Form {
            Section {
                ...
                .onChange(of: layoutPreset) {
                    PerformanceModeManager.apply(preset: layoutPreset)
                }

                Text(layoutPreset.description)
                    .font(.caption)
                    .foregroundStyle(.secondary)

                Defaults.Toggle(key: .performanceMode) {
                    Text("Performance mode")
                }
                .onChange(of: performanceMode) {
                    PerformanceModeManager.applyPerformanceMode(performanceMode)
                }
            } header: {
                Text("Quick setup")
            } footer: {
                Text("Balanced is recommended: sleek music, weather & calendar, without heavy battery drain.")
            }
...
struct About: View {
    ...
    var body: some View {
        VStack {
            Form {
                ...
                Section {
                    HStack(spacing: 30) {
                        Spacer(minLength: 0)
                        Button {
                            if let url = URL(string: "https://github.com/ryana79/notch-app") {
            ...
            VStack(spacing: 0) {
                Divider()
                Text("NotchPro — your notch, your rules")
                    .foregroundStyle(.secondary)
                    .padding(.top, 5)
                    .padding(.bottom, 7)
                    .multilineTextAlignment(.center)
...
struct Shelf: View {
    ...
    var body: some View {
        Form {
            Section {
                Defaults.Toggle(key: .notchProShelf) {
                    Text("Enable shelf")
                }
            ...
            Section {
                HStack(spacing: 16) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        ...
                        Text("App icon is set from the NotchPro bundle.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                    Spacer()
